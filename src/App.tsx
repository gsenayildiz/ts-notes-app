...
const App = () => {
  ...
  const createTag = (tag: Tag) => {
    // setTags([...tags, tag])  // iki kullanımda aynı sonucu verir
    setTags((prev) => [...prev, tag]); // prev de shep son değeri alır
  };

  // not oluşturma fonksiyonu
  const createNote = (noteData: NoteData): void => {
    // formdan gelen dataya ID ekleme
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/note/:id" element={<Layout notes={notes} />}>
          ...
          <Route
            path="edit"
            element={<Edit handleSubmit={updateNote} createTag={createTag} availableTags={tags} />}
          />
        </Route>
      </Routes>
    </BrowserRouter>
